/*
 * Copyright (C) 2011 Aivonic Design GmbH
 * All rights reserved.
 *
 * @Author: Thierry Reding <thierry.reding@avionic-design.de>
 * @Descr: Avionic Design PT-IP board net interface support functions
 *
...
int HYPHYReset(void)
{
	...
	goodacc = RMII_Write(MII_BMCR, BMCR_RESET);
	if (goodacc == 0)
		return 0;
...
int HWInit(bd_t * bd)
{
	...
	if (RMII_Write(MII_BMCR, (BMCR_SPEED100 | BMCR_ANENABLE)) == 0)
		return 0;
	...
	ENETMAC->sa [2] = (unsigned long) (enetaddr[0] | (enetaddr[1] << 8));
	ENETMAC->sa [1] = (unsigned long) (enetaddr[2] | (enetaddr[3] << 8));
	ENETMAC->sa [0] = (unsigned long) (enetaddr[4] | (enetaddr[5] << 8));
