...
static unsigned int hclkdivs[4] = { 1, 2, 4, 4 };

/* Post divider values for PLLs based on selected register value */
static unsigned int pll_postdivs[4] = { 1, 2, 4, 8 };
...
{
	unsigned long long i64freq;
	unsigned long long fcco;
	unsigned long long fref;
	unsigned long long cfreq;
	unsigned long long div_cfreq;
	unsigned long long div_fcco;
	unsigned long long div_fref;
	unsigned long long p, m, n;
	int mode;
...
unsigned int sys_get_rate(CLKPWR_BASE_CLOCK_T clkid)
{
	...
	if ((CLKPWR->clkpwr_sdramclk_ctrl & CLKPWR_SDRCLK_USE_DDR) != 0) {
		...
		if ((CLKPWR->clkpwr_hclk_div & CLKPWR_HCLKDIV_DDRCLK_NORM) != 0)
			ddr_hclk_div = 1;
		else if ((CLKPWR->clkpwr_hclk_div & CLKPWR_HCLKDIV_DDRCLK_HALF) != 0)
			ddr_hclk_div = 2;
	} else {
		/* SDRAM is being used */
